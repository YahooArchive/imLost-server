...
def get_profile(request):
    user = get_current_user(request)
    for k in user.keys():
        if k not in ['user_id','user_name','user_type','device_type','device_token','phone', 'location']:
            user.pop(k)
...
def get_contacts(request):
    ...
    if contacts is None:
        ...
        for c in contacts:
            if 'last_seen' in c and time.time() - c['last_seen'] < ONLINE_THRESHOLD:
                c['online'] = True
            else:
                c['online'] = False
            for k in ['device_token', 'device_type']:
                if k in c:
                    c.pop(k)
...
def add_contact(request):
    ...
    if contact_id == user['user_id']:
        return response_wrapper(403, 'You cannot add yourself as contact')
...
def accept_contact(request):
    ...
    for c in user['contacts']:
        if c['user_id'] == contact_id and c['status'] == 'pending':
            pending_contact = c

    if pending_contact is None:
        return response_wrapper(400, 'No such new contact')
...
def update_location(request):
    ...
    try:
        ...
        return response_wrapper(500, 'Illegal lat lng value')

    if not (abs(lng) <= 180 and abs(lat) <= 90):
        return response_wrapper(500, 'Illegal lat lng value')
